            ...
            while True:
                try:
                    ...
                    for i in range(len(test_results)):
                        test_results[i] = 1.0 if test_results[i] > 0.54 else 0.0
                    print(test_labs[:15])
                    print(test_results[:15])
                    acc = accuracy_score(test_labs, test_results)
                    score = roc_auc_score(test_labs, test_results)
                    print("model %s with accuracy %.4f" % (os.path.split(pb_file_path)[-1], acc))
